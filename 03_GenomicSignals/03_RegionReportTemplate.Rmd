...
sv_plots[["sv_victor"]]+geom_segment(data=select(One_Table,chr,start,end)%>%rename(`#CHROM`=chr,POS=start,END=end)%>%mutate(sample="TRD",TYPE="TRD"), size=5)
...
LP_data<-filter(LP_data, paste(cross,ID)%in% paste(pull(One_Table, cross), pull(One_Table, ID)))
